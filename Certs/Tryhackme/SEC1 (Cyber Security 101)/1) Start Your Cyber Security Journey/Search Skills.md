...
- Each vulnerability is assigned a unique identifier `CVE-YEAR-NUMBER`
- Sometimes they can receive monikers if they are impactful such as Heartbleed, React2Shell and Log4Shell
- These vulnerabilities have a CVSS score based on Impact, Complexity, and Availability. This is done to prioritise risk
- [ExploitDB](https://www.exploit-db.com/) is a Database of known and developed exploits along with Proofs of concept - [CVE](https://www.cve.org/) Lists all known vulnerabilities even if they are patched and no exploits exist

### Practical
- Once again we are in a mock vulnerability database where we must search the vulnerability `CVE-2026-1337` and retrieve the CVSS score.
  
  ![](Attachments/Pasted%20image%2020260629232135.png)
- The Score is 10 meaning this is a high risk vulnerability
...
### Practical
- It asks us to open a link to see their example page however, this can be done in Linux quite easily
...
- For example, if we wanted a proof of concept on a vulnerability or a scanner script or analysis of a vulnerability we can find it by searching GitHub in most cases.
